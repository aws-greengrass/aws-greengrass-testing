...
def method_lines(jmethod, jmethods, jlines):
    ...
    larger = list(
        int(jm.attrib.get('line', 0)) for jm in jmethods
...
def convert_lines(j_lines, into):
    ...
    for jline in j_lines:
        ...
        if mb + cb > 0:
            ...
            cline.set('condition-coverage',
                      percentage + ' (' + str(cb) + '/' + str(cb + mb) + ')')

            cond = ET.SubElement(ET.SubElement(cline, 'conditions'),
                                 'condition')
            cond.set('number', '0')
            cond.set('type', 'jump')
...
def convert_root(source, target, source_roots):
    target.set('timestamp',
               str(int(source.find('sessioninfo').attrib['start']) / 1000))
